...
# storage directories for LXC / Docker
...
## Docker: 
vm.docker.minversion = 1.10.0
vm.docker.maxversion = 1.99.0
...
# When using lxc/docker if this flag is set, QVD will assume that the home
# directories are not per virtual machine but per user and that they
# follow the typical NFS home structure: $NFSHOME/$USER
# Note that in order for this schema to work, the user ids used on the
# containers and in the directories should match. This is usually
# attained using an authentication plugin (i.e. LDAP) that assigns a UID
# that is the same used on the NFS.
# This needs to be mounted in the attribute path.storage.homefs
vm.container.home.per.user = 0
...
internal.hkd.docker.timeout.on_state.stopping.shutdown.waiting_for_docker = 180
internal.hkd.docker.timeout.on_state.stopping.stop.waiting_for_docker = 180
internal.hkd.docker.timeout.on_state.zombie.config.delaying = 60
internal.hkd.docker.timeout.on_state.zombie.reap.waiting_for_docker = 120
internal.hkd.docker.timeout.on_state.zombie.reap.delaying = 60
internal.hkd.docker.timeout.on_state.zombie.db.delaying = 60
